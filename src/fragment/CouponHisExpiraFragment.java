...
import fragment.CouponHisUsedFragment.backgroundGetCpList;
...
public class CouponHisExpiraFragment extends Fragment implements
	...
	class backgroundGetCpList extends AsyncTask<String, Void, String> {
		...
		protected String doInBackground(String... params) {
			...
			try {
				if(array.size() == 0)
				{
					array = GetJsonFromAPI.getCpList(API.getCPListExpration(email,
							String.valueOf(currentPage)));
					totalPage = GetJsonFromAPI.getTotalPage(API.getCPListExpration(
		...
		protected void onPostExecute(String result) {
			...
			if (MainActivity.timeout == 1) {
				...
				lv.setOnScrollListener(CouponHisExpiraFragment.this);
				lv.setSelection(positionGridView);
				lv.setOnItemClickListener(CouponHisExpiraFragment.this);
				waitingDialog.cancel();
				}
				catch (Exception e) {
					// TODO: handle exception
